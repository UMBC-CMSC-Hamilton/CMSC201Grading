...
__BASE_PUB_DIR = '/afs/umbc.edu/users/s/l/slupoli/pub/'
course_admin = 'eric8'
...
def create_assignment(roster_file_name, assignment, overwrite=False):
    with open(roster_file_name) as roster_file:
        ...
        os.popen(f'fs sa {assignment_path} system:anyuser read')
        ...
        for person, section in roster_reader:
            ...
            if person != course_admin:
                attempts = 0
                os.popen('fs sa {} {}:submit_cmsc201 none'.format(the_path, course_admin))
                # remove anyuser rl privileges
                os.popen('fs sa {} system:anyuser none'.format(the_path))
