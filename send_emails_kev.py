...
# MAJORS INSTRUCTORS
INSTRUCTORS = [ "shender2", "pranavv1", "eric8",  "abalasu1", "samit1"]
# NONMAJORS INSTRUCTORS
#INSTRUCTORS = ["shender2", "eric8"]
...
def set_up_instructors():
    ...
    for index in range(len(title_list)):
        ...
        for row in sectioning_lines[1:]:
            # todo: Jess 11/29 - allow there to be a "x,y,z" header, use different expression to gather this :0
...
def send_instructor_emails():
    for instructor in INSTRUCTORS:
        ...
        final_instructor["from"] = "Sean Henderson <shender2@umbc.edu>"
        final_instructor["to"] = TA_NAME_ID_DICT[instructor] + " <{}@umbc.edu>".format(instructor)
        instructor_email_address = instructor + "@umbc.edu"
        try:
            SMTP_SERVER.sendmail("jturner3@umbc.edu", [instructor_email_address], final_instructor.as_string())
...
def send_ta_emails():
    for ta in section_dict:
        ...
        final_ta["from"] = "Sean Henderson <shender2@umbc.edu>"
        final_ta["to"] = TA_NAME_ID_DICT[ta] + " <{}@umbc.edu>".format(ta)
        ta_email_address = ta + "@umbc.edu"
        try:
            SMTP_SERVER.sendmail("jturner3@umbc.edu", [ta_email_address], final_ta.as_string())               
            print(final_ta)
...
if(len(sys.argv) != 2):
    print("Wrong number of arguments! Usage: ./send_emails.py <assignment>")
    exit()
